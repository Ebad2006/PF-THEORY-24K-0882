...
int main()
{
    int numOfEmployees, numOfPeriod;
    printf("Enter number of Employees: ");
    scanf(" %d", &numOfEmployees);
    printf("Enter number of Evaluation Periods: ");
    scanf(" %d", &numOfPeriod);

    int **ratings = malloc(sizeof(int *) * numOfEmployees);
    for (int i = 0; i < numOfEmployees; i++)
    {
        ratings[i] = malloc(sizeof(int) * numOfPeriod);
    }

    inputEmployees(ratings, numOfEmployees, numOfPeriod);
    displayPerformance(ratings, numOfEmployees, numOfPeriod);

    printf("\nEmployee of the Year Index: %d\n", findEmployeeOfTheYear(ratings, numOfEmployees, numOfPeriod));

    printf("\nHighest Rated Period Index: %d\n", findHighestRatedPeriod(ratings, numOfEmployees, numOfPeriod));

    printf("\nWorst Rated Employee Index: %d\n", findWorstPerformingEmployee(ratings, numOfEmployees, numOfPeriod));
...
void inputEmployees(int **ratings, int numEmployees, int numPeriods)
{
    ...
    for (int E = 0; E < numEmployees; E++)
    {
        ...
        {
            printf("Enter Rating for Employee %d in period %d: ", E + 1, P + 1);
            scanf(" %d", &rate);
            if (rate >= 1 && rate <= 10)
            {
                ratings[E][P] = rate;
